...
def artist_works():     # painting from the famous artist (real target)
    ...
    labels = labels.astype(np.float32) # transform the value in labels to np.float32
...
with tf.variable_scope('Generator'):
    ...
    G_art = tf.concat((G_in, art_labels), 1)                    # combine ideas with labels in the second dimention -- N_IDEAS + label(1)
...
for step in range(7000):
    ...
    G_paintings = sess.run([G_out, prob_artist0, D_loss, train_D, train_G],    # train and get results
                                    {G_in: G_ideas, real_in: artist_paintings, art_labels: labels})[0]
